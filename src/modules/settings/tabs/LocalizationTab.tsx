...
export default function LocalizationTab() {
  ...
  return (
    <div className="p-5 max-w-3xl mx-auto space-y-5">
      ...
      <FieldCard title={t('localization.language')} hint="Choose how text appears on the kiosk screen.">
        <SettingsSelect
          id="lang-select"
          value={localization.locale}
          onChange={(e) => setLocalization({ locale: (e.target as HTMLSelectElement).value as typeof localization.locale })}
        >
      ...
      <FieldCard title={t('localization.currency')} hint="3-letter currency code used for prices and receipts.">
        <SettingsInput
          id="currency-input"
          type="text"
          value={localization.currency}
          onChange={(e) => setLocalization({ currency: (e.target as HTMLInputElement).value.trim().toUpperCase() })}
          placeholder="USD"
          maxLength={3}
      ...
      <FieldCard title={t('localization.timezone')} hint="Controls local time on receipts and order timestamps.">
        <SettingsSelect
          id="tz-select"
          value={localization.timezone}
          onChange={(e) => setLocalization({ timezone: (e.target as HTMLSelectElement).value })}
        >
          {TIMEZONES.map((tz) => (
      ...
      <FieldCard title={t('localization.dateFormat')} hint="Date layout used in staff-facing screens.">
        <SettingsSelect
          id="date-format-select"
          value={localization.dateFormat}
          onChange={(e) => setLocalization({ dateFormat: (e.target as HTMLSelectElement).value })}
        >
          {DATE_FORMATS.map((f) => (
      ...
      <div className="rounded-2xl p-5" style={{ background: themeColors.surface, border: `1.5px solid ${themeColors.border}` }}>
        <p className="text-sm font-bold font-brand" style={{ color: themeColors.text }}>{t('localization.preview')}</p>
        <div className="mt-3 space-y-2 text-sm font-brand">
          <p>
            <span style={{ color: themeColors.muted }}>{t('localization.priceLabel')}: </span>
            <span className="font-bold" style={{ color: themeColors.text }}>{samplePrice}</span>
          </p>
          <p>
            <span style={{ color: themeColors.muted }}>{t('localization.dateLabel')}: </span>
            <span className="font-bold" style={{ color: themeColors.text }}>{sampleDateTime}</span>
          </p>
          <p>
            <span style={{ color: themeColors.muted }}>Text direction: </span>
            <span className="font-bold" style={{ color: themeColors.text }}>
              {['ar'].includes(localization.locale) ? 'Right-to-left (RTL)' : 'Left-to-right (LTR)'}
            </span>
          </p>
        </div>
      </div>
